...
*	1. 更新AES算法支持
	...
	5. 更新非接模块多级联寻卡接口
*	===================== v1.03 ===============
*	1. 添加 NDK_SYS_ResumeEvent 接口说明
*   2. 添加 NDK_SecLoadDukptKey 接口说明
*	===================== v1.02 ===============
*	1. 添加 NDK_szGetCapability 接口说明
*	===========================================
*	1. 整合事件机制最新的类型定义，发布新版本NDK头文件以及编译环境内容
* 
*/
#ifndef NDK_H_
#define NDK_H_
...
#define SEC_KEYBLOCK_FMT_AES		(0x4145534B)	/**<AES密钥包格式,"AESK"*/
#define SEC_KEYBLOCK_FMT_CBC		(0x43424300)	/**<Extension of CBC mode cipher text key injection */

/**
 *@brief Structure for holding CBC mode cipher text key values. it is used when performing "SEC_KEYBLOCK_FMT_CBC" mode key injection, and ST_KEYBLOCK_CBC will be used as the value of "pblock" in ST_EXTEND_KEYBLOCK.
 */
typedef struct {
	unsigned int ivlen;			/**< length of IV*/
	char *iv;			/**< IV value for CBC encryption*/
	unsigned int len;			/**< length of cipher text key data*/
	char *input;				/**< pointer to cipher text key data*/
}ST_KEYBLOCK_CBC;
 ...
 *	注意：本结构不包含密钥记录的MAC值！
 */
typedef struct KEY_STORE_UNIT_S {
	unsigned int id;			/*密钥ID*/
	unsigned int arch;			/*密钥体系*/
	unsigned int type;			/*密钥类型信息*/
	unsigned int len;			/*密钥长度信息*/
	unsigned char value[0];		/*存储密钥值，可变长，具体使用时候分配，注意：必须放在结构体最末尾*/
} KEY_STORE_UNIT;
...
typedef enum{
	...
	SEC_MAC_AES,       /**< 数据分为16字节的block，不足补0，每个block进行SM4加密后与下一个block异或后按SM4加密*/
	SEC_MAC_SM4_UNIONPAY,/**银联卡sm4算mac模式**/
...
typedef enum {
	...
	SYS_HWINFO_GET_RFID_CHIP_TYPE,	/*<获取非接芯片类型*/
	SYS_HWINFO_GET_MAX, 
} EM_SYS_HWINFO;
...
typedef enum {
	...
	SYS_EVENT_TAMPER = 0x00000001,      /*安全触发*/
	SYS_EVENT_REBOOT = 0x00000002,      /*通知系统进行重启自检*/
...
typedef enum {
	...
	NDK_ERR_SECKLA_BASE = (-1500),                                    /**<未知错误*/
	NDK_ERR_SECKLA_ERR_INTERNAL = (NDK_ERR_SECKLA_BASE -1),           /*Unspecified internal error.*/
	NDK_ERR_SECKLA_INVALIDPARAM = (NDK_ERR_SECKLA_BASE -2),           /*Invalid parameter passed to function.*/
	NDK_ERR_SECKLA_ERR_INVALID_CRT = (NDK_ERR_SECKLA_BASE -3),        /*Invalid certification*/
	NDK_ERR_SECKLA_ERR_INVALID_SIG = (NDK_ERR_SECKLA_BASE -4),        /*Invalid nonce signature*/
	NDK_ERR_SECKLA_ERR_KEY_NOT_FOUND = (NDK_ERR_SECKLA_BASE -5),      /*Key not found*/
	NDK_ERR_SECKLA_ERR_INVALIDKEY_USAGE = (NDK_ERR_SECKLA_BASE -6),   /*Invalid use of the key according to the key tag*/
	...
	COM_FAIL= -6000,
	NDK_CALL_BACK =			-6010, /**<用于表示回调*/
	NDK_ERR_EMVUNSUPPORT = -10000,
}EM_NDK_ERR;
...
typedef enum {
	ICC_EMV_MODE = 1,			//EMV2000, auto adjust baud rate, nonsupport PPS
	ICC_BPS_576_MODE,			//EMV2000, baud : 57600, nonsupport PPS
	ICC_BPS_384_MODE,			//EMV2000, baud : 38400, nonsupport PPS
	ICC_BPS_192_MODE,			//EMV2000, baud : 19200, nonsupport PPS
	ICC_SSC_MODE,				//Social Security Card, auto adjust baud rate, nonsupport PPS
	ICC_ISO7816_PPS_MODE,		//ISO7816, baud : 9600, support PPS,
	ICC_ISO7816_BPS_96_MODE,	//ISO7816, baud : 9600, nonsupport PPS
	ICC_ISO7816_DEFAULT_MODE	//ISO7816, auto adjust baud rate, nonsupport PPS
}POWERUP_STANDARD;
...
/**
 *@brief	SM2非对称公私钥对中的公钥进行加密的功能（国密算法）
 *@param	eccpubkey		公钥
 *@param	Message			被加密数据
 *@param	MessageLen		数据长度
 *@param	Crypto			加密后密文数据
 *@param	CryptoLen		加密后数据长度
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法
 *@li	\ref NDK_ERR "NDK_ERR" 			操作失败
...
/**
 *@brief	SM2非对称公私钥对中的私钥进行解密的功能（国密算法）
 *@param	eccprikey		私钥
 *@param	Crypto			待解密密文数据
 *@param	CryptoLen		密文数据长度
 *@param	Message			解密后数据
 *@param	MessageLen		解密后数据长度
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法(pstPublicKey\pstPrivateKey为NULL)	
 *@li	\ref NDK_ERR "NDK_ERR" 			操作失败
...
/**
 *@brief	SM2非对称公私钥对中的私钥对摘要（32字节）计算数字签名（64字节）的功能（国密算法）
 *@param	eccprikey		私钥
 *@param	e				摘要数据
 *@param	output			签名数据
 *@return
...
/**
 *@brief	SM2非对称公私钥对中的公钥对数字签名（64字节）计算摘要（32字节）的功能（国密算法）
 *@param	pPublicKey		公钥
 *@param	e				摘要数据
 *@param	pSignedData		签名数据
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法(pstPublicKey\pstPrivateKey为NULL)	
 *@li	\ref NDK_ERR "NDK_ERR" 			操作失败
...
/**
 *@brief	生成SM2非对称公私钥对对应的数字摘要（32字节）的功能（注意：在该算法中，运算还需要用到ID、对应SM2非对称公私钥对的公钥等数据）（国密算法）
 *@param	usID			标签长度
 *@param	pID				标签内容
 *@param	usM				原数据长度
 *@param	pM				原数据
...
/**
 *@brief	根据SM3算法以分组方式（除最后一个分组以外，每个分组长度须为64字节整数倍）生成数字摘要（32字节）的功能（国密算法）
 *@param	pDat			数据
 *@param	len				更新数据长度
 *@return
 *@li	NDK_OK			操作成功
...
/**
 *@brief	根据SM3算法以分组方式注入最后一组数据后生成数字摘要（32字节）（国密算法）
 *@param	pDat			数据
 *@param	len				更新数据长度
 *@param	pHashDat		生成摘要
 *@return
...
/**
 *@brief	SM3算法以一次性传入数据方式生成数字摘要（32字节）（国密算法）
 *@param	pDat			数据
 *@param	len				数据长度
 *@param	pHashDat		生成摘要
 *@return
...
/**
 *@brief	SM4算法以ECB、CBC等模式进行对称密钥加解密运算
 *@param	pKey			密钥
 *@param	pIVector		初始向量值
 *@param	len				数据长度
 *@param	pSm4Input		待加解密数据
 *@param	pSm4Output		加解密生成数据
 *@param	mode			加解密模式
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法(pstPublicKey\pstPrivateKey为NULL)	
 ...
 *@li	\ref NDK_ERR_MACLLOC "NDK_ERR_MACLLOC"               内存空间不足
*/
extern int (*NDK_SecLoadDukptKey)(uchar ucGroupIdx, uchar ucSrcKeyType, uchar ucSrcKeyIdx, uchar ucKeyLen, uchar * psKeyValueIn, uchar * psKsnIn, ST_SEC_KCV_INFO * pstKcvInfoIn);
/**
 *@brief	获取DUKPT值
 ...
 *@li	
*/
extern int (*NDK_GetTamperStatus)(void);

/**
 *@brief	获取数据库秘钥表个数（不包含SNK秘钥表）
 *@param	num		返回秘钥表个数
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 				参数错误
 *@li	\ref ERR_SECKM_OPT_ERROR "ERR_SECKM_OPT_ERROR" 	数据库执行失败
 *@li	\ref ERR_SECKM_OPEN_DB "ERR_SECKM_OPEN_DB"		数据库打开失败
*/
extern int (*NDK_SecGetTableNum)(int * num);

/**
 *@brief	获取数据库所有秘钥表名（不包含SNK秘钥表）
 *@param	nSizeofTables		最大存储长度
 *@param	pszTables			返回查询到的秘钥表名
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 				  参数错误
 *@li	\ref ERR_SECKM_OPT_ERROR "ERR_SECKM_OPT_ERROR" 	  数据库执行失败
 *@li	\ref ERR_SECKM_OPEN_DB "ERR_SECKM_OPEN_DB"		  数据库打开失败
 *@li	\ref ERR_SECKM_SIZE_ERROR "ERR_SECKM_SIZE_ERROR"  buff长度不足
*/
extern int (*NDK_SecGetKeyTables)(int nSizeofTables, char * pszTables);
 ...
 *@li	\ref ERR_SECKM_OPT_ERROR "ERR_SECKM_OPT_ERROR" 	  	数据库执行失败
 *@li	\ref ERR_SECKM_OPEN_DB "ERR_SECKM_OPEN_DB"		  	数据库打开失败
 *@li	\ref ERR_SECKM_NOT_SUPPORT "ERR_SECKM_NOT_SUPPORT"  没有操作权限
 *@li	\ref ERR_SECKM_TABLE_ERROR "ERR_SECKM_TABLE_ERROR"  删除的秘钥表不存在
*/
extern int (*NDK_SecDropKeyTable)(char * table);
...
/**
 *@brief	设置IC卡上电模式
 *@param	pnMode		IC卡标准类型
 *@param	pnVoltage	IC卡GPIO值
 *@return
 *@li	NDK_OK				操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 			参数非法
 *@li	\ref NDK_ERR_IOCTL "NDK_ERR_IOCTL" 	驱动调用错误
*/
extern int (*NDK_IccSetPowerUpMode)(int pnMode, int pnVoltage);

/**
 *@brief	获取IC协议
 *@param	emIctype		IC卡类型
 *@param	pnProtocol		IC卡协议
 *@return
 *@li	NDK_OK				操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 			参数非法
 *@li	\ref NDK_ERR_IOCTL "NDK_ERR_IOCTL" 	驱动调用错误
*/
extern int (*NDK_IccGetProtocol)(EM_ICTYPE emIctype, int *pnProtocol);
...
*/
extern int  (*NDK_RfidVersion)(uchar *pszVersion);
 ...
 *@li	\ref NDK_ERR_IOCTL "NDK_ERR_IOCTL" 		驱动调用错误(射频驱动接口RFID_IOCG_M1ANTI调用失败返回)
*/
extern int (*NDK_M1Anti_SEL)(uchar ucSelCode, int *pnDataLen, uchar *psDataBuf);
/**
 ...
 *@li	\ref NDK_ERR_MI_Para "NDK_ERR_MI_Para" 	非接触卡-内部参数错			0xa9
*/
extern int (*NDK_M1Select_SEL)(uchar ucSelCode, int nUidLen, uchar *psUidBuf, uchar *psSakBuf);
...
*/
extern int  (*NDK_GetRfidType)(int *pnRfidType);
...
/**
 *@brief	在寻卡的基础上获取ATQA和ATQB
 *@param	psPicctype	卡片类型
 *@param	pnAtqlen	返回数据长度
 *@param	psAtqbuf	返回数据
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数错误
 *@li	\ref NDK_ERR_MI_NOTAGERR "NDK_ERR_IOCTL" 	驱动调用错误
*/
extern int (*NDK_RfidPiccDetect_Atq)(uchar *psPicctype, int *pnAtqlen, uchar* psAtqbuf);

/**
 *@brief	felica卡轮询接口
 *@retval	psRecebuf为轮询之后返回的felica数据，pnRecvlen轮询后felica返回的数据长度
 *          psRecebuf包含:出厂识别码(IDm)和出厂参数(PMm),这两个参数固化在芯片中,不得修改
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数错误(psRecebuf或者pnRecvlen为NULL)
 *@li	\ref NDK_ERR_OPEN_DEV "NDK_ERR_OPEN_DEV" 	设备文件打开失败(射频设备文件打开失败)
 *@li	\ref NDK_ERR_IOCTL "NDK_ERR_IOCTL" 		驱动调用错误(射频驱动接口IOCTL_RFID_FELICA_POLL调用失败返回)
 *@li	\ref NDK_ERR_RFID_NOCARD 没有寻到felica卡
*/
extern int (*NDK_RfidFelicaPoll)(uchar *psRecebuf,int *pnRecvlen);

/**
 *@brief	felica卡APDU交互,即读写卡过程
 *@param	nSendLen		发送的命令长度
 *@param	psSendBuf		发送命令缓冲区
 *@retval 	pnRecvLen	接收数据长度
 *@retval	psReceBuf	接收数据缓冲区
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法(psSendBuf/pnRecvLen/psReceBuf为NULL)
 *@li	\ref NDK_ERR_OPEN_DEV "NDK_ERR_OPEN_DEV" 	设备文件打开失败(射频设备文件打开失败)
 *@li	\ref NDK_ERR_IOCTL "NDK_ERR_IOCTL" 		驱动调用错误(射频驱动接口IOCTL_RFID_FELICA_APDU调用失败返回)
*/
extern int  (*NDK_RfidFelicaApdu)(int nSendlen, uchar *psSendbuf, int *pnRecvlen,  uchar *psRecebuf);

/**
 *@brief	Mifare 寻卡、防碰撞、选卡(寻卡类型必须已经设置为TYPE-A)
 *@param	ucReqCode	0=请求REQA, 非0=唤醒WUPA, 一般情况下需要使用WUPA
 *@retval 	psUID		UID数据缓冲区
 *@retval 	psUIDLen	UID数据长度
 *@retval 	psSak		最后一次选卡操作返回的SAK数据
 *@retval
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法(pnDataLen/psDataBuf为NULL)
 ...
 *@li	\ref NDK_ERR_MI_Para "NDK_ERR_MI_Para" 	非接触卡-内部参数错			0xa9
*/
extern int (*NDK_MifareActive)(uchar ucReqCode,uchar *psUID, uchar *pnUIDLen, uchar *psSak);

/**
 *@brief	M0卡 '页'读取操作
 *@param	ucPageNum	要读的起始页号
 *@retval	pnDataLen	读取的数据长度(返回16, Mifare UltraLight系列卡片的读操作返回16字节数据)
 *@retval	psPageData	读取的数据
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法(pnDataLen、psBlockData为NULL)
 *@li	\ref NDK_ERR_OPEN_DEV "NDK_ERR_OPEN_DEV" 	设备文件打开失败(射频设备文件打开失败)
 ...
 *@li	\ref NDK_ERR_MI_Para "NDK_ERR_MI_Para" 	非接触卡-内部参数错			0xa9
*/
extern int (*NDK_M0Read)(uchar ucPageNum, int *pnDataLen, uchar *psPageData);

/**
 *@brief	M0卡 '页'写操作
 *@param	ucPageNum	数据写入的页号
 *@param	pnDataLen	写入数据的长度(只支持传入4或者16, 分别对应Mifare UltraLight系列卡片的Write命令
 *@param				和CompatibilityWrite写命令，两种写命令有效写入数据都为4字节)
 *@param	psPageData	写入数据
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法(pnDataLen、psBlockData为NULL、pnDataLen非法)
 *@li	\ref NDK_ERR_OPEN_DEV "NDK_ERR_OPEN_DEV" 	设备文件打开失败(射频设备文件打开失败)
 ...
 *@li	\ref NDK_ERR_MI_Para "NDK_ERR_MI_Para" 	非接触卡-内部参数错			0xa9
*/
extern int (*NDK_M0Write)(uchar ucPageNum, int pnDataLen, uchar *psPageData);
 ...
 *@li	\ref NDK_ERR_MI_Para "NDK_ERR_MI_Para" 	非接触卡-内部参数错			0xa9
*/
extern int (*NDK_M0Authen)(uchar *psKey);
/*
**************************************************************************************************
***********************************                       ****************************************
***********************************      系统接口模块      ****************************************
...
/**
 *@brief  获取NDK能力
 *@param in   输入参数: nSizeOfCap - 传入的szCaps指针所指向的BUFFER的长度 (nSizeOfCap 目前至少需要大于 8个字节，后续将根据NDK能力值定义增加长度)
 *@param out  输出参数: szCaps - 能力值的字符串，“NNN” - 这样的指示值
 *@return 返回值
 *@li		NDK_ERR_PARA - 传入的nSizeOfCap < 8 或者 szCaps == NULL
 * 
 * 判断是否支持外卡：
 *		const char* caps = NDK_szGetCapability();
 *		if(strlen(caps) >= 3 && caps[2]==Y) {	
 *			// 当前版本支持外卡
 *	 	}
 *		else {
 *	 	 	// 当前版本不支持外卡
 ...
 *
*/
extern int (*NDK_SysGetCapability)(int nSizeOfCap, char* szCaps);
 ...
 *@brief  重启事件监听
 *@param in   输入参数：
 *@param 		EM_SYS_EVENT  eventNum, - 需要重启监听的事件
 *@return 返回值
 *@li			NDK_OK - 0  成功
 *@li			NDK_OK -   0 –    成功
 *@li			NDK_ERR_POSNDK_PERMISSION_UNDEFINED 权限未声明
 ...
 *@li			NDK_ERR_POSNDK_EVENT_INIT 初始化错
*/
extern int  (*NDK_SYS_ResumeEvent)(EM_SYS_EVENT eventNum);
/********emv**********/
extern const char *(*NAPI_EMVL2GetVersion)(void);

/**
* @Func: NAPI_CLL2_EntryPointGetVersion
* @brief: get Entrypoint Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(*NAPI_CLL2_EntryPointGetVersion)(void);

/**
* @Func: NAPI_CLL2_PaypassGetVersion
* @brief: get Paypass Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_PaypassGetVersion)(void);

/**
* @Func: NAPI_CLL2_PaywaveGetVersion
* @brief: get Paywave Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_PaywaveGetVersion)(void);

/**
* @Func: NAPI_CLL2_QpbocGetVersion
* @brief: get Qpboc Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_QpbocGetVersion)(void);

/**
* @Func: NAPI_CLL2_ExpresspayGetVersion
* @brief: get ExpressPay Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_ExpresspayGetVersion)(void);

/**
* @Func: NAPI_CLL2_DiscoverpayGetVersion
* @brief: get Discoverpay Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_DiscoverpayGetVersion)(void);

/**
* @Func: NAPI_CLL2_PureGetVersion
* @brief: get Pure Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_PureGetVersion)(void);

/**
* @Func: NAPI_CLL2_JCBGetVersion
* @brief: get JCB Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_JCBGetVersion)(void);

/**
* @Func: NAPI_CLL2_InteracGetVersion
* @brief: get Interac Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_InteracGetVersion)(void);

/**
* @Func: NAPI_CLL2_RupayGetVersion
* @brief: get Rupay Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_RupayGetVersion)(void);

/**
* @Func: NAPI_CLL2_MIRGetVersion
* @brief: get MIR Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_MIRGetVersion)(void);

/**
* @Func: NAPI_CLL2_MultibancoGetVersion
* @brief: get Multibanco Kernel Version
* @Param:
* @Return:
* @retval: Version
*/
extern const char *(* NAPI_CLL2_MultibancoGetVersion)(void);

extern int (*NAPI_EMVL2GetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_EntryPointGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_PaypassGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_PaywaveGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_ExpresspayGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_DiscoverpayGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_InteracGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_JCBGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_PureGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_RupayGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_QpbocGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_MIRGetKernelChecksum)(unsigned char *checksum, unsigned int size);
extern int (*NAPI_MultibancoGetKernelChecksum)(unsigned char *checksum, unsigned int size);
/**
 * 设置主动寻卡
 * 0关闭 1开启
 * */
extern int (*NDK_SetCheckinCardEventFlag)(int flag);
