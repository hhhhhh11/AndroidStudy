...
char * GetSectionAllString(char *pFilename,char *pSection)
{
	...
	int i,j,nFind,nSubstrlen=strlen(pSection)+2;//包含[]这两个字符的长度
	...
	pSub=(char *)malloc(nSubstrlen+3);	
	memset(pSub,0,nSubstrlen+3);	
	memset(szBuff,0,sizeof(szBuff));
	memset(szResult,0,sizeof(szResult));

	strcat(pSub,"[");
	strcat(pSub,pSection);
	strcat(pSub,"]");
	//CFunCommon::DlSprintf("pFilename = %s\n",pFilename);
	fp=fopen(pFilename,"rb");
	if (!fp) 

	{	
		CFunCommon::DlSprintf("Failed:open %s error\n",pFilename);
		return NULL;
	...
	nBuffSize=ftell(fp);	
	fseek(fp,0,SEEK_SET);	
	fread(szBuff,nBuffSize,1,fp);
	fclose(fp);

	i=0;
	j=0;
	//CFunCommon::DlSprintf("szBuff = %s\n",szBuff);
	//CFunCommon::DlSprintf("pSub = %s\n",pSub);
	while (1)
	{
		if (tolower(szBuff[i])==tolower(pSub[0]))
		{
			nFind=0;
			for (j=0;j<nSubstrlen;j++)
			{
				if (tolower(szBuff[i+j])!=tolower(pSub[j]))
				{
					break;
				}
				else
	...
	j=i+nSubstrlen;
	//CFunCommon::DlSprintf("j=%d,len=%d\n",j,strlen(szBuff));
...
//	CFunCommon::DlSprintf("%d:%d:%d\n",j,i+nSubstrlen+1,j-1-i-nSubstrlen);
	strncpy(szResult,szBuff+i+nSubstrlen+1,j-1-i-nSubstrlen); //拷贝值到数组，并去除尾部的换行符号
	free(pSub);
	//	CFunCommon::DlSprintf("szResult = %s\n",szResult);
	return szResult;
}
...
static int GetLine(char *buf, int buflen, char *content, char **rem1, char **rem2, char **nextline)
{
	...
	*rem2 = p - cntCR - cntLF;
	...
	i = (int)(cont2 - cont1);
	//CFunCommon::DlSprintf("cont2:%s\n",cont2);
	//CFunCommon::DlSprintf("cont1:%s\n",cont1);
	if (i >= SIZE_LINE)
		return LINE_ERROR;
...
//加载ini文件至内存
int  CIniRW::iniFileLoad(const char *filename)
{
	FILE *file;
	int len;
...
static int iniGetValue(const char *section, const char *key, char *value, int maxlen, const char *defvalue)
{
	...
	while (gBuflen - uselen > 0) {
		...
		type = GetLine(p, gBuflen - uselen, content, &rem1, &rem2, &nextline);
		uselen += (int)(nextline - p);
	//	CFunCommon::DlSprintf("content: %s\n",content);
	//	CFunCommon::DlSprintf("uselen=%d\n",uselen);
		if (LINE_SECTION == type) {
			if (found || section == NULL) break;		//发现另一section
			content[strlen(content) - 1] = 0;			//去尾部]
			StrStrip(content + 1);						//去首尾空格
...
//获取字符串，不带引号
int  CIniRW::iniGetString(const char *section, const char *key, char *value, int maxlen, const char *defvalue)
{
	int ret;
	int len;
...
//获取整数值
int  CIniRW::iniGetInt(const char *section, const char *key, int defvalue)
{
	char valstr[64];
...
//获取浮点数
double  CIniRW::iniGetDouble(const char *section, const char *key, double defvalue)
{
	char valstr[64];
...
int  CIniRW::iniSetString(const char *section, const char *key, const char *value)
{
	FILE *file;
...
int  CIniRW::iniSetInt(const char *section, const char *key, int value, int base)
{
	char valstr[64];
