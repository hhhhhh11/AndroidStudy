...
static void sha1_compress(union hash_state *md)
{
	...
	for (i = 0; i < 16; i++)
		W[i] = (((unsigned int) md->sha1.buf[(4 * i) + 0]) << 24) |
		       (((unsigned int) md->sha1.buf[(4 * i) + 1]) << 16) |
		       (((unsigned int) md->sha1.buf[(4 * i) + 2]) << 8) |
		       (((unsigned int) md->sha1.buf[(4 * i) + 3]) << 0);
	...
	/* compress */
	for (i = 0;  i < 20; i++)  {
		j = ROL(a, 5) + F0(b,c,d) + e + W[i] + 0x5a827999;
		e = d;
		d = c;
		c = ROL(b, 30);
		b = a;
	...
	for (i = 20; i < 40; i++)  {
		j = ROL(a, 5) + F1(b,c,d) + e + W[i] + 0x6ed9eba1;
		e = d;
		d = c;
		c = ROL(b, 30);
		b = a;
	...
	for (i = 40; i < 60; i++)  {
		j = ROL(a, 5) + F2(b,c,d) + e + W[i] + 0x8f1bbcdc;
		e = d;
		d = c;
		c = ROL(b, 30);
		b = a;
	...
	for (i = 60; i < 80; i++)  {
		j = ROL(a, 5) + F3(b,c,d) + e + W[i] + 0xca62c1d6;
		e = d;
		d = c;
		c = ROL(b, 30);
		b = a;
...
void sha1_done(union hash_state * md, unsigned char *hash)
{
	...
	/* append length */
	for (i = 60; i < 64; i++)
		md->sha1.buf[i] = (unsigned char)((md->sha1.length >> ((63 - i) * 8)) & 255);
	sha1_compress(md);

	/* copy output */
	for (i = 0; i < 20; i++)
		hash[i] = (unsigned char)((md->sha1.state[i >> 2] >> (((3 - i) & 3) << 3)) & 255);
}
