...
public class BytesUtils {
	...
	public static byte[] Ascii2Bytes(byte[] buf, int index, int length) {
		StringBuilder temp = new StringBuilder();
		for (int i = 0; i < length; i++) {
			temp.append((char) buf[index + i]);
		}
		String src =  temp.toString();
		int len = src.length();
		byte[] b = new byte[len / 2];
		for (int i = 0; i < len; i += 2) {
			// 两位一组，表示一个字节,把这样表示的16进制字符串，还原成一个字节
			b[i / 2] = (byte) ((Character.digit(src.charAt(i), 16) << 4) + Character
